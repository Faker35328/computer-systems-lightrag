...
from lightrag.operate import chunking_by_recursive_character, chunking_by_token_size
...
@pytest.mark.offline
def test_recursive_character_keeps_short_text_as_single_chunk():
...
@pytest.mark.offline
def test_recursive_character_prefers_markdown_and_paragraph_boundaries():
    tokenizer = make_tokenizer()
    content = "## A\n\n" + ("a" * 35) + "\n\n## B\n\n" + ("b" * 35)
    ...
    assert max(chunk["tokens"] for chunk in chunks) <= 45
    assert [chunk["chunk_order_index"] for chunk in chunks] == [0, 1]


@pytest.mark.offline
def test_recursive_character_preserves_markdown_blocks_when_under_limit():
    tokenizer = make_tokenizer()
    content = "\n\n".join(
        [
            "intro",
    ...
    assert all(chunk["tokens"] <= 55 for chunk in chunks)
    assert any(
        "```c" in chunk["content"]
        and "int main" in chunk["content"]
        and chunk["content"].rstrip().endswith("```")
        for chunk in chunks
    )
    assert any(
        "| a | b |" in chunk["content"] and "| 1 | 2 |" in chunk["content"]
        for chunk in chunks
    )
    assert any("$$" in chunk["content"] and "a=b+c" in chunk["content"] for chunk in chunks)
