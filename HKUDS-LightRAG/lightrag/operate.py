...
def chunking_by_recursive_character(
    ...
    """Split text on semantic-ish character boundaries before token fallback.

    The public signature matches ``chunking_by_token_size`` so it can be used as
    LightRAG's ``chunking_func``. It keeps Markdown-ish blocks intact when they
    fit in the token budget, then recursively tries weaker separators.
    """
    ...
    protected_blocks = _split_markdown_protected_blocks(content)
    separator_patterns: list[tuple[str, str]] = [
        ("markdown heading", r"(?m)(?=\n#{1,6}\s+)"),
        ("paragraph", r"\n\s*\n+"),
        ("line", r"\n"),
        ("chinese sentence", r"(?<=[。！？])"),
        ("english sentence", r"(?<=[.!?])\s+"),
        ("semicolon", r"(?<=[；;])"),
        ("comma", r"(?<=[，,])"),
        ("space", r"\s+"),
    ]
    ...
    for block in protected_blocks:
        ...
        joiner = "\n\n" if current else ""
        candidate = f"{current}{joiner}{block}" if current else block
        candidate_tokens = len(tokenizer.encode(candidate))
        if current and candidate_tokens > chunk_token_size:
            flush_current()
            current = block
        else:
            current = candidate
...
def _split_markdown_protected_blocks(content: str) -> list[str]:
    """Group fenced code, HTML tables, Markdown tables, and block formulae."""
    ...
    for line in lines:
        ...
        if mode == "formula":
            protected.append(line)
            if stripped.endswith("$$") and len(protected) > 1:
                flush_protected()
                mode = None
            continue
...
def _recursive_character_split(
    ...
    for separator_index, (separator_name, pattern) in enumerate(separator_patterns):
            ...
            if len(tokenizer.encode(piece)) > chunk_token_size:
                chunks.extend(
                    _recursive_character_split(
                        tokenizer,
                        piece,
                        separator_patterns[separator_index + 1 :],
                        chunk_token_size,
                        chunk_overlap_token_size,
                    )
                )
        ...
        if chunks and all(
            len(tokenizer.encode(chunk)) <= chunk_token_size for chunk in chunks
        ):
            return chunks

    logger.warning(
        "Recursive character chunk exceeds token limit; falling back to token window: len=%d limit=%d",
        len(tokenizer.encode(text)),
        chunk_token_size,
    )
    return [
        item["content"]
        for item in chunking_by_token_size(
            tokenizer,
            text,
            chunk_overlap_token_size=chunk_overlap_token_size,
            chunk_token_size=chunk_token_size,
